...
class InventoryApp:
    ...
    def view_inventory(self):
        try:
            cursor.execute("SELECT name, qty, price, last_updated_by, last_updated_on FROM inventory WHERE org_id = %s", (self.current_org_id,))
            ...
            for item in items:
                entry = f"Name: {item['name']}, Qty: {item['qty']}, Price: ₹{item['price']:.2f}, Updated By: {item['last_updated_by']}, On: {item['last_updated_on']}\n"
